...
export const createUser = async (req: Request, res: Response) => {
    try {
        ...
        const db = await resolveDB(final_tenant_id);
...
export const Login = async (req: Request, res: Response) => {
    try {
        ...
        const db = await resolveDB(tenant_id);
        const User = getUserModel(db);
        const existUser = await User.findOne({ email });
        if (!existUser) {
        ...
        const token = jwt.sign(
            { user_id: existUser._id, role: existUser.role, tenant_id },
            process.env.JWT_SECRET as string,
            { expiresIn: "15m" }
        );

        res.status(200).json({ message: "Login success", user: existUser, token });
    } catch (error: any) {
        res.status(500).json({ message: error.message || "internal server" });
    }
};
